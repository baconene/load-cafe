...
import { Search, X, ChevronRight, Menu as MenuIcon, ArrowLeft } from 'lucide-vue-next'
...
const allProducts = computed(() =>
    categories.value.flatMap(c => c.products.map(p => ({ ...p, category: { name: c.name } })))
)
...
const totalFiltered = computed(() => filteredCategories.value.reduce((n, c) => n + c.products.length, 0))
...
function getIcon(name: string, cat = '') {
    ...
    if (n.includes('mocha') || n.includes('coffee') || n.includes('brew')) return '☕'
    if (n.includes('matcha') || n.includes('tea') || c.includes('tea')) return '🍵'
    if (n.includes('cake') || n.includes('muffin') || n.includes('cookie') || n.includes('brownie')) return '🎂'
    if (n.includes('pastry') || n.includes('croissant') || n.includes('scone')) return '🥐'
    if (n.includes('sandwich') || n.includes('panini') || c.includes('food')) return '🥪'
    if (n.includes('frappe') || n.includes('shake') || n.includes('smoothie')) return '🥤'
    if (n.includes('juice') || n.includes('lemonade')) return '🍹'
    return '☕'
}

// ── Category tab scroll ref ───────────────────────────────────────────────────
...
onMounted(() => {
    ...
    gsap.utils.toArray<HTMLElement>('.reveal-up').forEach(el => {
        gsap.fromTo(el, { y: 40, opacity: 0 }, { y: 0, opacity: 1, duration: 0.75, ease: 'power3.out',
    ...
    <div class="min-h-screen bg-black text-white font-sans overflow-x-hidden">
        ...
        <nav class="fixed top-0 inset-x-0 z-50 border-b border-white/8"
            ...
            <div class="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 h-14 sm:h-16">
                ...
                <div class="flex items-center gap-2.5 shrink-0">
                    <div class="h-8 w-8 sm:h-9 sm:w-9 rounded-full overflow-hidden border border-white/20 flex items-center justify-center bg-white/5 shrink-0">
                        ...
                        <span v-else class="text-white font-black text-xs">☕</span>
                    </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Link v-else :href="login()"
                        class="hidden sm:flex rounded-full bg-white px-4 py-1.5 text-xs font-bold text-black hover:bg-white/90 transition-all duration-200">
                        Staff Login
                    </Link>
            ...
            <div v-if="mobileNavOpen"
                ...
                <div class="pt-2">
                    ...
                    <Link v-else :href="login()"
                        class="block text-center rounded-full bg-white px-4 py-2.5 text-sm font-bold text-black">
                        Staff Login
                    </Link>
                </div>
            </div>
        ...
        <section class="relative flex items-center pt-14 sm:pt-16 min-h-[55vh] sm:min-h-[75vh] pb-10 px-4 sm:px-6 lg:px-8 overflow-hidden">
            ...
            <div class="relative z-10 max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
                ...
                <!-- Cup visual (hidden on small phones, shown on sm+) -->
                <div class="hidden sm:flex justify-center lg:justify-end">
                    <div class="cup-float relative w-56 h-56 lg:w-72 lg:h-72 flex items-center justify-center">
                        ...
                        <div class="relative w-44 h-44 lg:w-60 lg:h-60 rounded-full flex items-center justify-center"
                            ...
                            <span class="text-[5.5rem] lg:text-[7.5rem] select-none">☕</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section id="menu" class="border-t border-white/8">
            ...
            <div class="sticky top-14 sm:top-16 z-40 border-b border-white/8"
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="py-3 flex items-center gap-3">
                        <div class="relative flex-1 max-w-xs sm:max-w-sm">
                            ...
                            <input
                                ...
                                class="w-full rounded-full border border-white/15 bg-white/5 pl-9 pr-8 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:border-white/30 focus:bg-white/8 transition-all"
                            />
                    ...
                    <div ref="tabsRef" class="flex gap-2 overflow-x-auto pb-3 scrollbar-hide -mx-1 px-1">
                        <button
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
                ...
                <div v-if="filteredCategories.length === 0" class="text-center py-20">
                    ...
                    <p class="text-white/40">No items match "{{ searchQuery }}"</p>
                    <button @click="searchQuery=''" class="mt-4 text-xs text-white/50 underline hover:text-white">Clear search</button>
                </div>
        ...
        <section id="experience" class="py-20 sm:py-28 px-4 sm:px-6 lg:px-8 border-t border-white/8">
            <div class="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                ...
                <div class="grid grid-cols-2 gap-3 reveal-up">
                    <div v-for="feat in [
                        { icon:'🌱', title:'Ethically Sourced', body:'Premium beans from certified sustainable farms.' },
                        { icon:'✋', title:'Handcrafted',       body:'Every drink prepared with precision by our baristas.' },
                        { icon:'🌡️', title:'Perfect Temp',      body:'Precise brewing temperatures for optimal flavor.' },
                        { icon:'💚', title:'Made with Love',    body:'Passion poured into every cup, every day.' },
                    ]" :key="feat.title"
                        class="rounded-xl border border-white/8 p-4 sm:p-5 hover:border-white/15 transition-colors"
                        style="background:rgba(255,255,255,0.03)">
                        <div class="text-2xl sm:text-3xl mb-2">{{ feat.icon }}</div>
            ...
            <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-5">
                <div class="flex items-center gap-2.5">
                    <div class="h-8 w-8 rounded-full overflow-hidden border border-white/15 flex items-center justify-center bg-white/5 shrink-0">
                        <img v-if="logoUrl" :src="logoUrl" class="h-full w-full object-contain" alt="Logo" />
                        <span v-else class="text-white font-black text-[10px]">☕</span>
                    </div>
                    ...
                    <div class="relative mx-5 mt-5 rounded-xl overflow-hidden border border-white/8"
                        ...
                        <div class="aspect-square flex items-center justify-center">
                            <img v-if="selectedProduct.image"
                                ...
                                class="w-full h-full object-cover" />
                            <span v-else class="text-[6rem] sm:text-[8rem]">
                                {{ getIcon(selectedProduct.name, selectedProduct.category?.name ?? '') }}
                            </span>
                        </div>
                    </div>
